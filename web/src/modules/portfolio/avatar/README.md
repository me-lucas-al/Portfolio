...
`tone`, `thinking`. No rect/camera/framing fields exist - a 2D
...
- `sprite-frames.ts` maps `(expression, mouthState)` to a PNG URL
...
- `web/public/avatar/sprites/*.png` - 8 frames (4 expressions × closed/open
  mouth), each a distinct AI-generated 2D illustration with a real
  transparent background. Generated by `web/scripts/generate-avatar-sprites.mjs`
  (not part of the app - see that script's own doc comment) from
  `ai-knowledge-base/avatar_lucas.jpg` as a reference image. Re-running that
  script (`pnpm --filter @portfolio/web generate:avatar-sprites`) writes
  straight back to these same 8 file names - the contract - without touching
  any code.
