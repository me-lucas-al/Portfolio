...
export function MediaFullscreenDialog({
  ...
  const dialogTitle = srTitle || (title ? `Modo tela cheia - ${title}` : "Visualização em tela cheia")
  const imageAlt = alt || title || "Imagem em tela cheia"
            ...
            {imagesUrl.map((fullUrl, fIndex) => (
              <CarouselItem key={fIndex} className="basis-full flex justify-center items-center h-full pl-0 relative">
                <div className="relative flex justify-center items-center w-full h-[80vh]">
                  <Image
                    src={fullUrl}
                    alt={`${imageAlt} - Tela cheia ${fIndex + 1}`}
                    width={1920}
                    height={1080}
                    className="w-full h-full object-contain rounded-xl shadow-2xl"
                    sizes="100vw"
